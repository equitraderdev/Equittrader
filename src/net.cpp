...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "EquiTrader " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"59.95.128.176", "59.95.128.176"},
    {"211.57.108.", "95.211.57.108."},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. EquiTrader is probably already running."), addrBind.ToString().c_str());
        else
