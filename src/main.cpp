...
#if defined(NDEBUG)
# error "EquiTrader cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa97a8cb54beeee2a2716fe601202a79526e87e5a02a58a48e9a0926f6dce36ad");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EquiTrader: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EquiTrader Signed Message:\n";
...
{
    // EquiTrader: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 40 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 7200000 * COIN;
            }

    // Subsidy is cut in half every 810000 blocks
    nSubsidy >>= (nHeight / 810000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 100 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EquiTrader: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EquiTrader: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1492098610, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "EQUITRADER";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 40 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f2f05f28798613da0c5a5df1edd955d649c6b0af0c73954451c619a2ea214b48304ec0204f3928c1c4adaef06888929e9ab8396b09706a9ede811d9127d146bf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1492098610;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5758140;
        ...
        assert(block.hashMerkleRoot == uint256("0x10fce8c85e3ff693e5d677f6471a13e06c2985bb1ac8c083d06c473894e4f140"));
...
unsigned char pchMessageStart[4] = { 0x71, 0x61, 0xfe, 0x83 }; // EquiTrader: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EquiTraderMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EquiTraderMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EquiTraderMiner(CWallet *pwallet)
{
    printf("EquiTraderMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("equitrader-miner");
    ...
    try { loop {
        ...
        printf("Running EquiTraderMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EquiTraderMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EquiTraderMiner, pwallet));
}
